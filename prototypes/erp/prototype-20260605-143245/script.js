...
const selectDefaults={productKeyword:"商品 ID",goodsKeyword:"货品 ID",store:"全部全托管店铺",category:"全部类目",stocking:"全部备货类型",stock:"全部库存状态",binding:"全部绑定状态",sort:"创建时间倒序",mapping:"全部",syncStatus:"全部",ruleMethod:"全部",logPageSize:"10 条/页"};
...
function logsForProduct(product){
  ...
  return Array.from({length:87},(_,index)=>{
    const minute=30-Math.floor(index/3),second=(40-index*7+60)%60;
    return {time:`2026-06-08 15:${String(Math.max(0,minute)).padStart(2,"0")}:${String(second).padStart(2,"0")}`,module:modules[index%modules.length],content:contents[index%contents.length],operator:index%4===0?"系统任务":product.operator};
  });
}
function renderLog(){
  const logs=logsForProduct(currentLogProduct),totalPages=Math.ceil(logs.length/logPageSize),start=(logPage-1)*logPageSize,end=Math.min(start+logPageSize,logs.length);
  document.getElementById("logRows").innerHTML=logs.slice(start,end).map(log=>`<tr><td>${log.time}</td><td>${log.module}</td><td>${log.content}</td><td>${log.operator}</td></tr>`).join("");
  document.getElementById("logRangeText").textContent=`第 ${start+1}-${end} 条 / 共 ${logs.length} 条`;
...
document.onclick=()=>{document.querySelectorAll(".c-select__popup.open").forEach(x=>x.classList.remove("open"));document.getElementById("categoryPopup").classList.remove("open")};
...
document.getElementById("collapseBtn").onclick=()=>{document.querySelector(".c-shell").classList.toggle("collapsed");showMessage(document.querySelector(".c-shell").classList.contains("collapsed")?"菜单已收起":"菜单已展开")};
...
document.getElementById("logJumpInput").onkeydown=e=>{if(e.key==="Enter"){const total=Math.ceil(87/logPageSize),page=Number(e.currentTarget.value);if(page>=1&&page<=total){logPage=page;renderLog()}else showMessage(`请输入 1-${total} 的页码`)}};
