...
| 当前时间口径：发货时间 / 结算时间 | PRD 4.3 / 用户补充 | 是 | 默认发货时间 |
| 筛选项：平台、店铺、平台 SKU、发货 SKU、发货时间、结算时间、是否结算 | PRD 4.2 / 4.3 / 10.3A | 是 | 平台 SKU 和发货 SKU 都可筛选 |
| 汇总指标 | PRD 10.2A / 10.3 | 是 | 展示订单数、结算数、未结算数、收入、利润差异 |
| 主表核心字段 | PRD 4.2 / 10.3 | 是 | 避免 40+ 字段全部铺到主表 |
...
| 时间口径 | DateComparisonControl 临时组合 | RangePicker / Segmented | selected / default |
| 汇总区 | MetricComparisonCard | Card / Statistic | neutral / risk |
...
| 操作 | 触发位置 | 交互结果 |
|---|---|---|
| 切换时间口径 | 发货时间 / 结算时间 Segmented | 列表、汇总和导出统一按当前口径计算；结算时间口径自动排除未结算订单 |
| 筛选 | 筛选区按钮 | 按平台、店铺、SKU、时间、是否结算过滤 |
| 重置 | 筛选区按钮 | 清空所有筛选并恢复发货时间口径、全部结算状态 |
